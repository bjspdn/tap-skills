...
2. **Discovery.** List `.tap/tickets/<slug>/` containing ≥1 `task-*.md` AND no `.tap/tickets/done/<slug>`. Lex-sort slugs. If user passed args (`/tap-run a b`), filter to those slugs in lex order. Resolve `quality_gates`: `.tap/config.json#qualityGates` → CLAUDE.md `## Quality gates` → ask user.
...
4. **Per ticket — open worktree.** `git -C <main> worktree add .tap/worktrees/<slug> -b tap-<slug>`. Capture `parent_sha = git -C <wt> rev-parse --short HEAD`. Set `commit_lock = <wt>/.git/tap-commit.lock`. Parse trailers — `git -C <wt> log <parent_sha>..HEAD --format=%B` — and build the resume-skip set keyed on `(Tap-Task, Tap-Phase)`.
...
- `commit_lock` — absolute path to `<worktree_path>/.git/tap-commit.lock`
