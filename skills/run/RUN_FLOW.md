...
1. **Preflight.** `git status --porcelain` on the main repo — must be empty, else halt and surface dirty paths.
...
3. **Planning.** For each ticket, read every `task-NN-*.md` frontmatter (`id`, `files.create`, `files.modify`, `context[]`); skip bodies. Infer waves (see Wave inference). Render plan (ticket order, per-ticket waves, gate commands; surface any pair forced into a later wave by file overlap). Check for stale worktree references via `git worktree list` — if any stale entries exist, include them in the plan output: "Stale worktree references will be pruned on approval: [list]". Present plan and ask engineer to approve. Reject → halt. On approval: run `git worktree prune` if stale entries were listed, then seed TaskList (see Task tracking below).
   - **Task tracking.** After approval, create a TaskList reflecting the full execution plan. Use `TaskCreate` to build a nested structure: ticket → wave → task → phase (RED/GREEN/REFACTOR). Wire cross-wave dependencies with `TaskUpdate(addBlockedBy=…)`. Mark each task `in_progress` when its first phase dispatches; mark `completed` when its last phase commits successfully (or mark `failed` if saga-isolated). This TaskList is the run's live progress tracker — it MUST exist before step 4 begins.
