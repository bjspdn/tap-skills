...
|--------------------|--------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Ingestion          | "I remember the ideation from the conversation, no need to re-read"            | Uses stale conversation context instead of what ideation.md actually says. Drift compounds downstream | Read `.tap/tickets/{slug}/ideation.md` end-to-end. Disregard prior conversation context — the file is the source of truth                                     |
| Ingestion          | "File paths in the ideation look right, no need to verify"                     | Trusts file:line refs without grounding. Paths may be wrong or stale                                  | Run targeted Grep/Read on every file:line reference before slicing. Do not invent file paths                                                                  |
| Dependency scan    | "Simple feature, dependency scan is overkill"                                  | Misses consumer chains and integration seams. Tasks land without wiring                               | Always dispatch DependencyScanner. Simple features still have consumers. Skipping is how orphaned optionals happen                                            |
| Pattern scan       | "No patterns apply here, skip it"                                              | Misses neighbor conventions. GREEN produces code that doesn't compose with its surroundings           | Always dispatch PatternScanner. "No pattern found" is a valid result — not dispatching is not                                                                 |
| Slicing            | "Types + implementation + tests as separate tasks is cleaner"                  | Horizontal slicing. Tasks can't pass independently, can't commit independently                        | Each task is a vertical slice: one behavior, end-to-end. RED + GREEN + REFACTOR in every task                                                                 |
| Slicing            | "This is small, one big task covers everything"                                | Under-slicing. Task touches 3+ concerns, agent burns 40+ turns, high failure rate                    | If a task modifies files across more than 2 services, split it. One concern per task                                                                          |
| Expand-contract    | "Only a few consumers, I can update them all in one task"                       | Breaks intermediate commits. Shared module change + consumer migration in one task = non-compilable midpoint | Shared/high-fanout file with public API break = expand-contract. No shortcuts based on "only a few" consumers                                            |
| Emission           | "Context frontmatter is verbose, the executing agent can find symbols itself"  | Agent wastes turns exploring codebase for symbol locations. Multiplied across every task in the ticket | Every referenced symbol gets a context entry with path, line, and signature. Verbose frontmatter saves downstream turns                                       |
| Emission           | "REFACTOR: improve structure and clean up"                                     | Vague refactor action. Agent flails for 40+ turns with no concrete target                            | Name specific operations with concrete file:symbol targets. If nothing needs refactoring, write "No refactoring needed" — don't invent vague work             |
| Emission           | "Line 42 is more precise than the function name"                               | Line numbers go stale on first edit. Anchor becomes meaningless before the task even runs             | Anchor by symbol name (function, class, exported binding), never by line number                                                                               |
| Integration-check  | "All tasks look complete, integration-check is redundant"                      | Misses orphaned provider chains. Data doesn't flow end-to-end, silent production failure             | Walk every provider-chain from the integration map. Every hop with `has-access='false'` must have a task. No shortcut                                         |
| Self-review        | "I just wrote these tasks, I know they cover the ideation"                     | Writing and reviewing are different cognitive modes. Self-review catches gaps the author is blind to  | Re-read ideation.md. Check every intent, signature, error case, constraint, boundary against the task list. Mechanically, not from memory                     |
| Independent audit  | "Self-review passed, the audit agent is just overhead"                         | Self-review shares this conversation's context — if the ideation converged on a wrong abstraction, self-review can't catch it | Always dispatch IndependentAuditor. It sees only the files, no shared context. That's the point                                         |
| Placeholders       | "I'll fill in the details when more context is available"                      | TODO/TBD tokens reach the final file. Executing agent hits a placeholder and stalls or invents       | Every Action, Example, Verify, Done must contain concrete content. If you can't write it, the task is under-specified — resolve the gap first                  |
