...
- [Web Search Reference](dorks.md) — Brave Search operators, domain filtering, query patterns by domain
...
<phase name="context-detection">
  <purpose>
    Build a context block that scopes all subsequent research queries.
    For project-bound topics, detect the stack. For pure-knowledge topics,
    derive context from the user's prompt.
  </purpose>
  <steps>
    <step name="classify">
...
<phase name="research-loop">
  <purpose>
    Iterative multi-hop research. Each hop searches, analyzes findings,
    identifies gaps, and decides whether to continue or converge.
    Hard cap: 5 hops. Typical: 3.
  </purpose>

  <step name="classify-domain">
  ...
  <step name="hop">
    Each hop spawns 1-3 concurrent Explore agents depending on how many
    ...
    Agent(
      ...
      description: "<topic> — hop {N}: <specific angle>",
      prompt: "
        You are researching a technical topic. Your job is to find
        authoritative, current information.
        <context>{context_block}</context>
        <domain>{classified_domain}</domain>
        <topic>{research_topic}</topic>
        <angle>{specific_angle_for_this_hop}</angle>
        <prior_findings>{summary_of_what_previous_hops_found}</prior_findings>
        ## Search Strategy
        Use WebSearch & WebFetch. Use the [dorks](dorks.md) for query
        construction. Scope every query to the domain and context:
        - Use the domain-specific authoritative domains from the classification
        - For project-bound topics, include language/framework name in queries
        - For pure-knowledge topics, include domain-specific terminology
        - Cross-reference across at least 2 sources per claim
        - Prefer primary sources (specs, papers, official docs) over secondary
          (blog posts, tutorials, forum answers)
        ## What NOT to search
        - Do not re-search topics covered in <prior_findings>
        - Do not search generic tutorials — find design rationale and primary sources
        - Do not search results older than 2 years unless they are
          foundational references (e.g., original RFC, seminal paper, spec)
        ## Return format
        <findings>
        ...
        <convergence_signal>
          yes | no — did this hop produce substantially new information
          beyond what <prior_findings> already contained?
        </convergence_signal>
        <sources>
...
<phase name="crossref">
  <purpose>
    Cross-reference findings. For project-bound topics, ground findings in
    the codebase. For pure-knowledge topics, cross-reference sources against
    each other for agreement and contradiction.
  </purpose>
  <steps>
    <step name="route">
      Check the `<context>` block's `<mode>`:
      - **project-bound** → run `<step name="codebase-scan">`
      - **pure-knowledge** → run `<step name="source-crossref">`
    </step>
    <step name="codebase-scan">
      (Project-bound only)
      Agent(
        subagent_type: "Explore",
        description: "Cross-reference <topic> with codebase",
        prompt: "
          You have research findings about <topic> for the <context> ecosystem.
          Your job: determine how the current codebase relates to these findings.
          <knowledge_state>{final_knowledge_state}</knowledge_state>
          For each **Established** and **Probable** finding, scan the codebase:
          1. Does the codebase already follow this convention? (grep for patterns)
          2. Does it explicitly diverge? (different approach to the same problem)
          3. Is it absent? (the codebase hasn't addressed this concern yet)
          ## Return format
          <crossref>
          ...
          Rules:
          - Scan exports and usage patterns, not entire files
          - Do not read node_modules, vendor/, or third-party code
          - file:line references mandatory for every claim
          - Hard cap: 400 words
        "
      )
    </step>
    <step name="source-crossref">
      (Pure-knowledge only)
...
<phase name="synthesis">
  <purpose>
    Produce the research artifact. In standalone mode, emit a file.
    In callable mode, return structured findings to the calling context.
  </purpose>
  <steps>
    <step name="write">
