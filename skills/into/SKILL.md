...
| Decomposition      | "I'll stub the other tickets later / I can track them in context"            | Deferred tickets exist only in conversation memory — context compaction or session end loses them     | Stub ALL deferred tickets to disk immediately after decomposition. `ls .tap/tickets/` must match the full roadmap before picking a ticket to ideate                |
